...
//                 : "border-transparent"
//             }`}
//             onClick={() => router.push(path + "?active=leaderboard")}
//           >
//             Leaderboard
//           </button>
//         </div>
...
function SpecificDAO({ props }: { props: { daoDelegates: string } }) {
  ...
  const tabComponents: Record<string, React.ReactNode> = {
    delegatesList: <DelegatesList props={props.daoDelegates} />,
    delegatesSession: <DelegatesSession props={props.daoDelegates} />,
        ...
        <div className="hidden lg:flex gap-12 bg-[#D9D9D945] pl-16">
          {tabs.map((tab) => (
            <button
              key={tab.value}
              className={`border-b-2 py-4 px-2 ${
                searchParams.get("active") === tab.value
                  ? "text-blue-shade-200 font-semibold border-b-2 border-blue-shade-200"
                  : "border-transparent"
              }`}
              onClick={() => handleTabChange(tab.value)}
