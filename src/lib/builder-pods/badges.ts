...
const triggerToSlug: Record<BadgeTrigger, string> = {
    ...
    lab_participant_approved: 'builder_lab_participant',
...
export async function awardBadgeOnEvent(
    ...
    if (result && !result.easUid) {
        const mustAttestNow = trigger === 'pod_member_approved' || trigger === 'lab_participant_approved' || trigger === 'manual_assignment';
        if (mustAttestNow) {
