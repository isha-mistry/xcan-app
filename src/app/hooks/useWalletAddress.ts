...
export const useWalletAddress = (): WalletAddressHook => {
  ...
  const [walletAddress, setWalletAddress] = useState<string | null>(() => {
    ...
    return typeof window !== "undefined" ? getStoredWalletAddress() : null;
  });
  ...
  >(() => {
    return typeof window !== "undefined"
      ? (safeLocalStorage.getItem("addressSource") as
          | "external"
          | "embedded"
          | null)
      : null;
  ...
  const persistWalletAddress = useCallback(
    (address: string | null, source: "external" | "embedded" | null) => {
      if (address) {
  ...
  useEffect(() => {
    ...
    if (storedAddress && ready) {
      ...
      setAddressSource(
        safeLocalStorage.getItem("addressSource") as "external" | "embedded"
      );
      return;
    }
