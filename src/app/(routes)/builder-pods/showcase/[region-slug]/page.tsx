...
export default function RegionalShowcasePage() {
    ...
    const statusColors: Record<string, string> = {
        ...
        rejected: "bg-red-500/10 text-red-400/70 border-red-500/10",
    };

    return (
        <div className="max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-10 py-6">
            <Link href="/builder-pods/showcase" className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-white/80 hover:text-white/80 font-robotoMono mb-6 transition-colors">
            ...
            {isLoading ? (
                ...
                <div className="glass-container rounded-2xl p-12 text-center">
                    ...
                    <p className="text-sm text-white/80 font-robotoMono">Showcase not found</p>
                </div>
            ) : (
                <>
                    {/* Showcase Header */}
                    ...
                    >
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold font-robotoMono ${statusColors[showcase.status] || 'bg-white/5 text-white/50'}`}>
                                {showcase.status}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-[10px] text-white/75 font-robotoMono">
                            ...
                            {showcase.prizePoolUsd > 0 && (
                                <div className="flex items-center gap-1">
                                    <Award className="w-3 h-3 text-yellow-400/60" />
                        ...
                        {showcase.description && (
                            <p className="text-xs text-white/50 font-robotoMono mt-3 max-w-2xl leading-relaxed">
                                {showcase.description}
                            </p>
                        )}
                    </motion.div>
                    ...
                    {submissionsData?.submissions?.filter((s: any) => s.status === "winner").length > 0 && (
                        ...
                        >
                            <div className="flex items-center gap-2 mb-4">
                                <Medal className="w-4 h-4 text-yellow-400/70" />
                                <h2 className="text-[11px] font-bold uppercase tracking-[0.2em] text-yellow-400/70 font-robotoMono">
                                    Winners
                                </h2>
                            </div>
                    ...
                    >
                        ...
                        {!submissionsData?.submissions?.length ? (
                            <div className="glass-container rounded-2xl p-10 text-center">
                                <p className="text-xs text-white/50 font-robotoMono">No submissions yet</p>
                            </div>
                        ) : (
    ...
    return (
        ...
        >
            <div className="flex items-start justify-between mb-3">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold font-robotoMono shrink-0 ${statusColors[sub.status] || "bg-white/5 text-white/50"}`}>
                    {sub.placement ? `#${sub.placement}` : sub.status}
                </span>
            </div>

            {sub.projectSnapshot?.problemStatement && (
                <p className="text-[10px] text-white/45 font-robotoMono mb-3 line-clamp-2 leading-relaxed">
                    {sub.projectSnapshot.problemStatement}
                </p>
            )}
            ...
            <div className="flex items-center gap-2">
                {sub.githubRepo && (
                    <a
                        ...
                        className="flex items-center gap-1 px-2 py-1 rounded-lg bg-white/5 hover:bg-white/10 text-[9px] text-white/80 font-robotoMono transition-all"
                    >
