...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let query: any = {
      "meetings.status": "active", // Base condition for active meetings
    };
    ...
    const isEmptyQuery =
      Object.keys(query).length === 1 && query["meetings.status"];
    ...
    const ongoing: Array<Meeting & { host_address: string }> = [];
    const upcoming: Array<Meeting & { host_address: string }> = [];
    const recorded: Array<Meeting & { host_address: string }> = [];
    const hosted: Array<Meeting & { host_address: string }> = [];
    const attended: Array<Meeting & { host_address: string }> = [];
