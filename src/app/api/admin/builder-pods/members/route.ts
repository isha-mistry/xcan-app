...
import {
  ...
  recalculatePodScore,
  SCORE_WEIGHTS,
} from "@/lib/builder-pods/leaderboard";
...
import { sendPodMemberInvite } from "@/lib/builder-pods/pod-member-invite/service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const membersEnhanced = members.map((m: any) => {
      const activeContractsDeployed =
        activeDeploymentsByWallet.get(m.walletAddress) ?? 0;
      ...
      const totalScore =
        (m.stylusModulesCompleted || 0) *
          SCORE_WEIGHTS.individual.moduleCompleted +
        ...
        (m.projectContributionScore || 0) *
          SCORE_WEIGHTS.individual.projectContribution;
    ...
    return NextResponse.json(
      { success: true, members: membersEnhanced },
      { status: 200 },
    );
  } catch (error: any) {
...
export async function PATCH(req: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          error:
            ...
            "Invalid member approval payload",
        },
        { status: 400 },
      );
    }
    ...
    if (action === "approve" || action === "activate") {
      ...
      if (action === "approve") {
        const badgeTrigger = member.joinedViaQr
          ? "pod_member_approved"
      ...
      const approvedRole = member.requestedRole;
      member.requestedRole = null;

      if (approvedRole === "pod_member") {
        const result = await sendPodMemberInvite(member, adminWallet);
        if (!result.ok) {
          member.requestedRole = approvedRole;
          return NextResponse.json(
        ...
        await AuditLog.create({
          ...
          entityType: "PodMember",
          entityId: memberId,
          oldValue: { role: oldRole, requestedRole: approvedRole },
        ...
        return NextResponse.json(
          {
            ...
            member: {
              ...
              podMemberInviteStatus: member.podMemberInviteStatus,
            },
            invitePending: true,
          },
          { status: 200 },
      ...
      if (member.role === "pod_lead") {
        ...
        const podLeadPlatformRole = await PlatformRole.findOne({
          slug: "pod_lead",
        }).lean();
      ...
      return NextResponse.json(
        {
          success: true,
          member: { _id: member._id, role: member.role, status: member.status },
        },
        { status: 200 },
      );
